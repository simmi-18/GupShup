  ...
  return (
    <main className="flex-1 overflow-y-auto px-3 py-4 space-y-3 sm:space-y-4">
      {messages.map((msg, index) => {
        ...
        const rawFileName = msg.file_url?.split("/").pop() || "";
        ...
        const isExternalUrl = msg.file_url?.startsWith("http");
        const fileUrl = isExternalUrl
          ? msg.file_url
          : `${process.env.REACT_APP_API_URL}/gupshup/uploads/${msg.file_url}`;

        const isImage =
          msg.file_url?.match(/\.(jpeg|jpg|png|gif|webp)$/i) ||
          msg.file_url?.includes("gif") ||
          msg.file_url?.includes("image");

        const hasText = !!msg.message?.trim();
        const hasFile = !!msg.file_url?.trim();
